...
Status: **Phase 0 complete (0A audit + 0B external research + 0C synthesis).**
Describes what exists today and the target architecture, now grounded in the
reference research under [`research/`](./research/).

Audit date: 2026-08-07 (0A), updated 2026-08-08 (0B/0C).
Audited by: repository inspection (filesystem, build files, ROM tooling, artifact
availability probes) plus source-level research of fourteen reference
repositories — see [`research/REFERENCES.md`](./research/REFERENCES.md) for
commits, licenses and reuse verdicts.
...
```
bomb/
+-- BOMB_PLAN.md          master plan (product scope, rules, phases)
+-- CLAUDE.md             agent instructions
+-- README.md
+-- settings.gradle.kts   :preview, :app-preview
+-- gradle/               libs.versions.toml — Kotlin 2.3.20 / CMP 1.10.3 / MIUIX 0.8.8
+-- preview/              shared Compose (KMP android library) — design system + screens
+-- app-preview/          thin Android application packaging module
+-- docs/                 ARCHITECTURE.md, BOMB_PLAN.md, ROM_INTEGRATION.md,
                          SYSTEM_PATCHES.md, research/
```

Facts as of 2026-08-08:

- **Git repository**, branch `main`, 2 commits (UI shell + master plan).
- **Builds.** `./gradlew :app-preview:assembleDebug` produces a debug APK;
  package `com.hzzmonet.zkbomb`, minSdk 33, targetSdk 37.
- **44 Kotlin files**, all UI: the `Bomb*` design system over MIUIX, navigation
  with predictive back, and feature screens driven by a state holder.
- **No tests anywhere.** No `src/test`, no CI, no lint configuration.
- No AIDL, no native code, no `sepolicy/`, no `init/`, no `permissions/`.
- No privileged code of any kind: no service, no Binder surface, no backends.

So the UI shell exists and the *product* does not. Every subsystem — Task Manager,
Stats, Freeze, App Control, Component/Process Control, App Visibility, Settings
Virtualization, AdBlock/DNS/Firewall, Performance, Battery Lab, Rules, Bridge,
Recorder — is greenfield below the UI layer.

The `preview`/`app-preview` module names are historical (they date from the
removed browser preview target). They should be renamed to `:app` + the real
module split (§6, `BOMB_PLAN.md` §4) when privileged work starts.
...
## 7. Framework patch points (result of Phase 0B research)

The master plan added four subsystems that cannot be built from a priv-app alone:
App Visibility, Settings Virtualization, framework spawn policy, and
platform-integrated freeze. Source-level research located each one, and the result
is smaller than expected — **three files, four insertion points.**

| Subsystem | File (AOSP `1cdfff5`) | Insertion point | Note |
| --- | --- | --- | --- |
| App Visibility | `services/…/pm/AppsFilterBase.java` | `shouldFilterApplication(...)` :333 | single chokepoint for enumeration, direct lookup, UID mapping, and all four intent-resolution kinds — [`AOSP_PACKAGE_MANAGER.md`](./research/AOSP_PACKAGE_MANAGER.md) |
| Settings Virtualization | `packages/SettingsProvider/…/SettingsProvider.java` | `getSystemSetting` :1916, `getSecureSetting` :1654, `getGlobalSetting` :1465 | AOSP already does per-caller virtualization here for SSAID — [`AOSP_SETTINGS_PROVIDER.md`](./research/AOSP_SETTINGS_PROVIDER.md) |
| Framework spawn policy | `services/…/am/ProcessList.java` | `startProcessLocked(String processName, …)` :2650 | existing refusal contract returns `null` — [`AOSP_PROCESS_MANAGEMENT.md`](./research/AOSP_PROCESS_MANAGEMENT.md) |
| Deep freeze | `services/…/am/CachedAppOptimizer.java` | `freezeProcess(...)` :2292 veto points | Bomb expresses intent; the platform owns the mechanism (binder-freeze first) |

Each patch is a minimal diff against a known upstream commit, keeps its Apache-2.0
header, and delegates its decision to a Bomb policy object that holds an immutable
snapshot behind a `volatile` reference — no I/O, no allocation, no locking on these
paths.

`services.jar` patching is **ROM mode only**. Root mode cannot provide these four,
and `BombCapabilities` must report them unsupported there rather than silently
doing nothing.

## 8. Capability matrix by execution mode

Derived from the research notes; each row cites where it was established.

| Capability | ROM mode | Root mode | Established in |
| --- | --- | --- | --- |
| Task Manager (`/proc`) | yes | yes | AOSP process mgmt §5 |
| Foreground/background state | yes (`RunningAppProcessInfo.importance`) | yes | STATS_REFERENCES §2 |
| Force stop | yes (`FORCE_STOP_PACKAGES`) | yes | AOSP process mgmt §1 |
| Disable / enable app | yes | yes | HAIL §1 |
| Suspend / hide app | needs device-owner or platform reach → **probe** | probe | HAIL §1-2 |
| Deep freeze (cgroup freezer) | yes, via framework hook | mechanism reachable, races the platform → not recommended | AOSP process mgmt §3 |
| Framework spawn policy | yes | **no** | AOSP process mgmt §2 |
| Component control (PM) | yes | yes | BLOCKER §2 |
| Component control (IFW) | yes (probe write access) | yes | BLOCKER §3 |
| App Visibility | **yes** | **no** | AOSP PM §6 |
| Settings Virtualization | **yes** | **no** | AOSP Settings §8 |
| AdBlock / DNS | probe (resolver integration) | probe | ADGUARD_HOME |
| GPU / thermal / charge telemetry | probe per node | probe per node | STATS_REFERENCES §2 |
| HyperIsland bridge | three-signal probe | same probe, likely denied | HYPERBRIDGE §1 |
| Live Updates bridge | yes (API ≥ 16) | yes | LIVE_UPDATES §1 |
| Phone recording | probe capture, not permission | probe | BCR §3 |
| VoIP recording | **unsupported until demonstrated** | unsupported | BCR §3 |

"Probe" means: attempt the operation or read the node at runtime and report the
result. It never means "assume yes".

---

Module mapping, phase-by-phase file lists, the SELinux and AIDL plans, risks and
the test strategy are in [`BOMB_PLAN.md`](./BOMB_PLAN.md). Reference research is in
[`research/`](./research/).
