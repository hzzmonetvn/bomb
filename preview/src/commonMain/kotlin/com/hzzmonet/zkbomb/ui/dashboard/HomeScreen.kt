...
fun LazyListScope.homeContent(
    ...
    item {
        ...
        BombHeaderBanner(
            appVersion = install.versionDisplay,
            ...
            // Detected, not chosen. This row used to follow a ROM/Root toggle
            // the user could flip, so it reported "Integrated · priv-app +
            // bombd" on a plain sideloaded install — a claim about the device
            // that came from a preference.
            modeLabel = when (service.runtimeMode) {
                "ROM" -> "ROM"
                "ROOT" -> "Root"
                else -> if (install.privileged) "Privileged" else "Normal app"
            },
            modeDetail = when {
                // A ROM that declares the flag but shipped the APK outside
                // priv-app is a real and otherwise near-invisible mistake, so it
                // is called out rather than smoothed over.
                service.runtimeMode == "ROM" && !install.privileged ->
                    "Declared, but not in priv-app"
                service.runtimeMode == "ROM" -> "priv-app · ${connectionLabel(service)}"
                service.runtimeMode == "ROOT" -> "Module backend · ${connectionLabel(service)}"
                install.privileged -> "priv-app · not declared by the ROM"
                else -> "${install.slot} · no privileged permissions"
            },
            modeColor = when {
                service.runtimeMode == "ROM" && !install.privileged -> BombTheme.colors.critical
                service.runtimeMode != "NORMAL" || install.privileged -> BombTheme.colors.ok
                else -> BombTheme.colors.warn
            },
...
/** Short form of the service connection, for the Home banner's one-line detail. */
private fun connectionLabel(service: BombServiceState): String = when (service.connection) {
    BombConnection.CONNECTED -> "service connected"
    BombConnection.CONNECTING -> "connecting"
    BombConnection.UNAVAILABLE -> "service unavailable"
    BombConnection.NOT_SUPPORTED -> "service not supported"
}
